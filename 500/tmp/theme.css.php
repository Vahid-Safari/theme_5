...
/*    width: 40px;
    height: 40px;*/
    &.box4_icon_1{background-image:url(<?php echo $box4_icon_1;?>);}
    &.box4_icon_2{background-image:url(<?php echo $box4_icon_2;?>);}
    &.box4_icon_3{background-image:url(<?php echo $box4_icon_3;?>);}
    &.box4_icon_4{background-image:url(<?php echo $box4_icon_4;?>);}
	}
}
...
<?php ob_start();eval(" ?".">".$tpl->compileTemplate( php_clean(file_get_contents("/Applications/MAMP/htdocs/kcms5/code/themes/theme_5/css/menu.less")), null)."<?"."php ");$compiled = ob_get_contents();ob_end_clean(); echo "/* css/menu.less */\r\n$compiled;" ?>
...
.social{
   text-align: right;
  margin-top: 10px;
	<?php echo $social_style["css"];?>
	li{
		display: inline-block;
		a{
			display: block;
			width: <?php echo $social_item_size;?>px;
			font-size: <?php echo $social_item_size;?>px;
			box-sizing: content-box;
			line-height: 1;
			text-align: center;
			&.icon-social{
				<?php echo $social_item_style["css"];?>
				&:hover{
					opacity: 0.7;
					<?php echo $social_item_hover_style["css"];?>
				}	
			}
 ...
 */
footer{
	#copyright{float: <?php echo $xalign;?>;}
}
...
/* <?php if( $module_product || $module_shop ){ ?>{_include="css/product.less"}<?php } ?> */
...
color:#fff;
	...
	#floatbasket{
		...
		width: 300px;
		top: 100%;
		<?php echo $basket_dropdown_style["css"];?>
	}
}
...
.product{
	.product_body {
		padding: 10px 0;
		h3{
			font:<?php echo $font_weight;?> 14px/22px Font1,Arial;
			padding: 10px 0;
			border-bottom: 1px solid <?php echo $border_color;?>;
		}
	}
	h2{
		font:<?php echo $font_weight;?> 20px/36px Font1,Arial;
		border-bottom: 1px solid <?php echo $border_color;?>;
		padding: 10px 0;
	}
	...
	.product_image {
		...
		margin-bottom: 5px;
		img {
			/*cursor:url('<?php echo $global_url;?>images/magnify.cur'), pointer ;*/
	...
	.product_price{
		.product_status{
			...
			font:<?php echo $font_weight;?> 14px/30px Font1,Arial;
			border-bottom: 1px solid <?php echo $border_color;?>;
			padding: 10px 0;
		}
		span{
		...
		.old_price{
			...
			color: <?php echo $product_price_off_color;?>;
		}
	}
	...
	.attr_list {
		padding-<?php echo $align;?>: 10px;
		li{
			list-style-type:square;
			margin-<?php echo $align;?>:10px;
		}
	}
	.product_options .options{margin:0}
	.products_index li a{
		display:block;color:#000;font:<?php echo $font_weight;?> 17px/20px Font1,Arial;
		span{float:<?php echo $xalign;?>}
	}
	.btn-alertme{
		float:<?php echo $xalign;?>;
	}
...
.product_thumbs {
	...
	&:nth-child(3n+1) {margin-<?php echo $align;?>:0;}
	&:nth-child(3n+3) {margin-<?php echo $xalign;?>:0;}
}
...
.bank_logo{
	width:50px;
	height:50px;
	background: transparent url('<?php echo $global_url;?>images/banks.png') no-repeat center center;
...
.page_cart{
	...
	.basket_title{
		...
		.product_variant,.product_code{
			font: bold 12px/30px Font1, Arial;
			b{
				color:#37bb5c;
			}
		}
...
@arrow-sep: 15px;
@arrow-size: 25px;
@badge-active-bg: @nav_color;
@badge-active-color: #fff;
@nav-disabled-link-color: #eee;
@nav-link-hover-bg: darken(@nav_color, 0%);
@nav-link-padding: 4px;
@nav-pills-active-link-hover-bg: darken(@nav_color, 20%);
...
.nav-wizard {
	...
	> li {
		...
		&:not(:first-child) > div {
			/*	padding-right: 75px; 2*@arrow-size - @arrow-sep;*/
		...
		&.active > div {
			...
			&:after {
				border-right-color: @nav-pills-active-link-hover-bg;
			}
