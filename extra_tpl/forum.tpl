								...
								<li id='forum_{$value.id}' class='subforum_list row'>
									<div class='subforum_title col-lg-10 col-md-10 col-sm-10 col-xs-12 '>
										<div class='subforum_details'>
											<span class='subforum_icon'><i {if="$value.icon"}style="background-image:url('{$value.icon}');" class='custom_icon'{else}class='fa fa-comment'{/if}></i></span>
...
{if="$show_topics"}
	...
	{if="$topic_list"}
			...
			{loop="topic_list"}
				<li id='topic_{$value.id}' class='topic_list{if="$value.important"} topic_important{/if} row'>
							<div class='topic_title col-lg-7 col-md-7 col-sm-7 col-xs-10'>
								<span class='topic_icon'><i class='fa fa-file'></i></span>
							...
							<div class='topic_last col-lg-3 col-md-3 col-sm-3 hidden-xs'>
									{if="$value.last_user"}توسط {$value.last_user} <a href='{$value.last_link}' ><i class='fa fa-circle-arrow-left'></i></a><br/>{$value.last_date}{else}<div class='topic_no_answer'>بدون پاسخ</div>{/if}
							</div>
				</li>
			{/loop}
		</ul>
		...
		{if="$new_topic_link"}<a class='btn btn-custom btn-lg btn-new-topic' href='{$new_topic_link}'><span class='icon fa-plus icon-plus'></span> موضوع جدید</a>{/if}
	{else}
			{if="$new_topic_link"}
				<div class='content_btn' >
					<p>هیچ موضوعی در این بخش مطرح نشده است</p>
					<a class='btn btn-custom btn-lg' href='{$new_topic_link}'><span class='icon fa-plus icon-plus'></span> موضوع جدید</a>
				</div>
			{/if}
	{/if}
{/if}
