...
function block_special_products($side='side_right')
{
	global $_, $_db, $lang;
...
$block =  get_cache("special_products" ,  SITE_ID,0);
if(!$block)
{
  $block="-";
	load_function('shop');
	...
	set_cache("special_products" ,$block, YEAR,  SITE_ID,0);
}
if($block != "-")
{
  	add_theme_box($side, "block", "box_special_product", array(
			"title"=>" محصولات ويژه ",
			"text"=>$block ,
		));
}
