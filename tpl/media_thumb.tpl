...
{loop="list"}
	...
	{if="$value.show_media_type && $value.icon"}<div class='media_icon'><span class='glyphicon glyph{$value.icon}'></span></div>{/if}
	...
	<div class='media_info clearfix'>
		{if="$value.show_hit"}<a class='media_hit' href='{$value.link}'><span class='glyphicon glyphicon-eye-open'></span><strong>{$value.hit}</strong></a>{/if}
		{if="$value.show_like"}<a class='media_like' href='{$value.link}' data-like='{$value.id}'><span class='glyphicon glyphicon-heart'></span><strong>{$value.like}</strong></a>{/if}
		{if="$value.show_comments"}<a class='media_comment' href='{$value.link}#box_comments'><span class='glyphicon glyphicon-comment'></span><strong>{$value.comments}</strong></a>{/if}
	</div>
	{/if}
	</div>
</div>
